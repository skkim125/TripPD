...
struct AddPlaceMapView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .bottomSheet(bottomSheetPosition: $sheetHeight, switchablePositions: [.relativeBottom(0.15), .absolute(365), .relativeTop(0.78)], headerContent: { SearchListHeaderView(viewModel: viewModel, sheetHeight: $sheetHeight, isSelected: $isSelected, isSearched: $isSearched)}) {
                    if isSelected {
        ...
        .popup(isPresented: $showAddPlacePopupView) {
            RoundedRectangle(cornerRadius: 12)
                ...
                .overlay {
                    PlaceFormView(
                        schedule: $viewModel.output.schedule,
                        isSelectedPlace: $viewModel.output.isSelectedPlace,
                        showAddPlacePopupView: $showAddPlacePopupView,
                        setRegion: $setRegion,
                        viewType: .add
                    )
                }
